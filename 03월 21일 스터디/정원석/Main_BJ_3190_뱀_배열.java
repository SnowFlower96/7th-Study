...
public class Main_BJ_3190_뱀_배열 {
...
//		int dc_L[] = { -1, 0, 1, 0 };
		...
		while(true) {
			...
			int mr = r + dr[move_snake];
			int mc = c + dc[move_snake];

			if(mr < 1 || mr > N || mc < 1 || mc > N || map[mr][mc] == 2) {
				break;
			...
			if(map[mr][mc] == 4) {
				snake.offer(new Pos(mr, mc));
				map[mr][mc] = 2;
			}else {
				map[mr][mc] = 2;
				snake.offer(new Pos(mr, mc));
				
				map[snake.peek().r][snake.poll().c] = 0;
			}
